...
export default function CartPage() {
  const { items, removeFromCart, clearCart, total, addToCart } = useCart();
  ...
  const handleSuccess = () => {
    ...
    // Create order structure from cart items
    placeOrder({
                ...
                >
                  {items.length > 0 ? items.map((item, idx) => (
                    <motion.div
                      ...
                      className="bg-white p-4 rounded-[2rem] flex gap-6 items-center shadow-sm group hover:shadow-md transition-all border border-transparent hover:border-gray-100"
                    >
                      <div className="w-28 h-28 rounded-2xl relative overflow-hidden flex-shrink-0 bg-gray-100">
                        <Image src={item.image} alt={item.name} fill className="object-cover" />
                      </div>

                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-start mb-2">
                          <h3 className="font-bold text-xl truncate pr-4">{item.name}</h3>
                          <button
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-3 bg-gray-50 rounded-lg p-1">
                            <button className="w-6 h-6 flex items-center justify-center bg-white rounded shadow-sm text-xs hover:bg-gray-100"><Minus size={12} /></button>
                            <span className="font-bold text-sm w-4 text-center">{item.quantity}</span>
                            <button className="w-6 h-6 flex items-center justify-center bg-white rounded shadow-sm text-xs hover:bg-gray-100"><Plus size={12} /></button>
                          </div>
                        </div>
                      </div>
                    </motion.div>
              ...
              {step === 'cart' ? (
                ...
                <div className="space-y-3">
                  <button
                    ...
                    className="w-full py-5 rounded-full bg-black text-white font-bold text-lg hover:scale-[1.02] transition-transform shadow-xl shadow-black/20"
                  >
                    Place Order (COD)
                  </button>
                  <button
