      ...
      <div className="container px-4 md:px-6 z-10 grid md:grid-cols-2 gap-8 items-center">
        <div className="space-y-6">
          <AnimatePresence mode="wait">
            <motion.div
              key={slide.id}
              initial={slide.id === 1 ? { opacity: 1, x: 0 } : { opacity: 0, x: -50 }}
            ...
            >
              ...
              <h1 className="text-4xl md:text-5xl lg:text-7xl font-extrabold tracking-tighter leading-none mb-4">
                {slide.title}
              </h1>
              ...
              <div className="flex gap-4">
                <Link href={slide.ctaLink}>
                  <button className="px-6 py-3 md:px-8 md:py-4 rounded-full bg-primary text-white font-bold text-base md:text-lg hover:scale-105 transition-transform flex items-center gap-2 shadow-lg shadow-primary/25">
                    {slide.cta} <ArrowRight size={20} />
                  </button>
                </Link>
                {slide.id === 1 && (
                  <Link href="/custom">
                    <button className="px-6 py-3 md:px-8 md:py-4 rounded-full bg-white/50 dark:bg-white/10 backdrop-blur-sm border border-white/20 font-bold text-base md:text-lg hover:bg-white/80 dark:hover:bg-white/20 transition-colors">
                      Design Your Own
                    </button>
                  </Link>
                )}
        ...
        <div className="relative hidden md:flex items-center justify-center">
          <AnimatePresence mode="wait">
            <motion.div
              key={slide.id}
              initial={slide.id === 1 ? { opacity: 1, scale: 1, rotate: 0 } : { opacity: 0, scale: 0.8, rotate: 5 }}
